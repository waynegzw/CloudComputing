...
answer_2() {
	...
	awk '{if(NR==1) {min=$1; title=$2}};{if($1<min) {min=$1; title=$2;} else if($1==min){if($2>title) {min=$1; title=$2}}};END{print title "\t" min}' output
}
...
answer_3() {
        ...
        awk '{if(substr($20, 10)>=max) {max=substr($20, 10); title=$2}};END{print title "\t" max}' output
    }
...
answer_4() {
        ...
        awk '{if(substr($3, 10)=="0") {if($1>max){max=$1; title=$2}}};END{print title}' output
    }
...
answer_8() {
        ...
        awk '{if ($2~/\([0-9]{4}_film\)/ || $2~/\(film\)/) print $2}' output
    }
