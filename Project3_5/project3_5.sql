...
-- start query2_opt
select sum(lo_revenue), d_year, p_brand1 from lineorder_opt, part_opt, supplier_opt, dwdate_opt
where lo_orderdate = d_datekey 
and lo_partkey = p_partkey 
and lo_suppkey = s_suppkey
and p_category = 'MFGR#12' 
...
-- start query3_opt
select c_city, s_city, d_year, sum(lo_revenue) as revenue from 
lineorder_opt, customer_opt, supplier_opt, dwdate_opt
where years=1997 
...
credentials 'aws_access_key_id=;aws_secret_access_key=/6uUgdZtv' gzip compupdate off;
...
copy customer from 's3://awssampledb/ssbgz/customer' 
credentials 'aws_access_key_id=AKIAI6R7J5BI3A2ERG7A;aws_secret_access_key=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' gzip;
